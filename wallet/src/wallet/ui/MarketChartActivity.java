...
public class MarketChartActivity extends Activity {
    ...
    private void showMoreIntervalsDialog() {
        ...
        String[] intervalValues = {"1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h", "6h", "12h", "1D", "1W", "1M"};
        int[] intervalLabels = {
        ...
        for (int i = 0; i < intervalValues.length; i++) {
            ...
            final String intervalToLoad = interval.equals("1D")? "1d" : interval.equals("1W")? "1w" : interval;
    ...
    private void setupTimeframeChips() {
        ...
        String[] labels = {
                ...
                getString(R.string.interval_1d),
                getString(R.string.interval_1M),
                getString(R.string.more)
        };
        String[] real = {"15m", "15m", "1h", "4h", "1d", "1M", "1M"};

        for (int idx = 0; idx < labels.length; idx++) {
            String label = labels[idx];
            String realInterval = real[idx];
            TextView tv = new TextView(this);
            tv.setText(label);
            tv.setTextSize(13f);
            ...
            if (label.equals(getString(R.string.interval_15m)) && currentInterval.equals("15m")) {
                isSelected = true;
            }
            if (label.equals(getString(R.string.interval_1h)) && currentInterval.equals("1h")) {
                isSelected = true;
            }
            if (label.equals(getString(R.string.interval_4h)) && currentInterval.equals("4h")) {
                isSelected = true;
            }
            if (label.equals(getString(R.string.interval_1d)) && (currentInterval.equals("1d") || currentInterval.equals("1D"))) {
                isSelected = true;
            }
            if (label.equals(getString(R.string.interval_1M)) && currentInterval.equals("1M")) {
                isSelected = true;
            }
            ...
            tv.setOnClickListener(v -> {
                if (selectedLabel.equals(getString(R.string.time)) || selectedLabel.equals(getString(R.string.more))) {
    ...
    private void setupChartListener() {
        ...
        marketChartView.setOnChartUpdateListener(new MarketChartView.OnChartUpdateListener() {
            ...
            public void onTickerUpdate(float high24h, float low24h, float volBtc, float volUsdt, float changePercent) {
                runOnUiThread(() -> {
                    if (textHigh24h!= null) {
                        textHigh24h.setText(getString(R.string.chart_high_default).replace("--", String.format(Locale.US, "%.2f", high24h)));
                    }
                    if (textLow24h!= null) {
                        textLow24h.setText(getString(R.string.chart_low_default).replace("--", String.format(Locale.US, "%.2f", low24h)));
                    }
